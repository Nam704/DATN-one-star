    ...
    $('#applyFilter').on('click', function(e) {
      ...
      if (minPrice && maxPrice) {
        ...
        if (isNaN(min) || isNaN(max)) {
          ...
          errors.price = 'Giá tối đa phải lớn hơn hoặc bằng giá tối thiểu.';
        }
      }
      ...
      if (fromDate && toDate) {
        if (toDate < fromDate) {
          errors.dateRange = 'Ngày kết thúc phải sau hoặc bằng ngày bắt đầu.';
        }
      }
      ...
      if (Object.keys(errors).length > 0) {
        ...
        $.each(errors, function(key, msg) {
          ...
          switch (key) {
            ...
            case 'price':       $field = $('input[name="max_price"]'); break;
            case 'dateRange':   $field = $('input[name="created_to"]'); break;
            case 'dateFuture':  $field = $('input[name="created_to"]'); break;
            case 'sort_view':   $field = $('#sort_view'); break;
          }
          if ($field && $field.length) {
            $field.addClass('is-invalid');
            $('<small class="text-danger validation-error">'+msg+'</small>').insertAfter($field);
          }
        });
        return;  // Dừng xử lý
      }
