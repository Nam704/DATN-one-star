...
<<<<<<< HEAD
                                ...
                                <!-- Quill editor -->
                                <div id="snow-editor" style="height: 300px; background: #fff;">
                                    {!! $product->description !!}
                                </div>
...
=======
                                ...
                                <option value="variants" {{ $product->product_type == 'variants' ? 'selected' : '' }}>
...
<div class="tab-pane fade" id="attributes" role="tabpanel">
    <div>
        <!-- Header Buttons -->
        <div class="mb-2">
            <label for="attributeSelect" class="form-label">Chọn Thuộc Tính (nhiều)</label>
            <select id="attributeSelect" class="form-select">
                <!-- Attributes will be populated via JavaScript -->
            </select>
        </div>

        <div id="container-accords" class="accordion">
            <!-- Existing product attributes will be loaded here -->
            @if($product->productVariants && count($product->productVariants) > 0)
    @foreach($product->productVariants as $variant)
        <!-- Your existing variant code -->
    @endforeach
@endif
    ...
    <div class="d-flex justify-content-end mt-3">
        <button type="button" id="save-attribute-btn" class="btn btn-primary">
            Lưu chọn thuộc tính
        </button>
    </div>
</div>
...
<div class="tab-pane fade" id="variations" role="tabpanel">
    <div class="button-container">
        <button type="button" id="add-new-var" class="btn btn-outline-primary mb-2">
            Thêm mới biến thể
        </button>
        <button type="button" id="save-var" class="btn btn-primary mb-2">
            Lưu Biến thể
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="category_select" name="id_category">
                                    ...
                                    <option value="{{ $category->id }}" {{ $category->id == $product->id_category ?
                                        'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                    @foreach ($categories as $sub)
                                    @if ($sub->id_parent == $category->id)
                                    <option value="{{ $sub->id }}" {{ $sub->id == $product->id_category ? 'selected' :
                                        '' }}>
                                        -- {{ $sub->name }}
                                    </option>
                                    @endif
                ...
                @foreach($categories as $category)
                    <div class="category-item" id="category_{{$category->id}}">
                        <label class="toggle-subcategories" data-category-id="{{$category->id}}">
                            <input type="checkbox" 
                                   id="checkbox-category-id-{{$category->id}}" 
                                   name="categories[]" 
...
=======
                            <input name="image-primary" type="file" id="productImage" class="form-control mb-3" accept="image/*">
                            @if($product->image_primary)
                                <div id="currentImage" class="text-center">
                                    <img src="{{ asset('storage/' . $product->image_primary) }}" 
                                        alt="Current Product Image" class="img-fluid mb-2">
                                </div>
                            @endif
...
=======
                            <input type="file" id="albumImages" name="images[]" class="form-control mb-3" 
                                accept="image/*" multiple style="display: none;">
                                <div id="albumPreview" class="row g-2">
                                    @if($product->images && count($product->images) > 0)
                                        @foreach($product->images as $image)
                                            <div class="col-6 col-md-3">
                                                <div class="position-relative">
                                                    <img src="{{ asset('storage/' . $image->url) }}" 
                                                        alt="Album Image" class="img-fluid rounded">
...
@push('scripts')
<script>
    let productAlbums = @json($product->product_albums);
...
<script src="{{ asset('admin/api/renderTabRight.js') }}"></script>
<script src="{{ asset('admin/api/updateProduct.js') }}"></script>


@endpush
=======
            <input type="text" id="product-type" name="product_type" 
                value="{{ old('product_type', $product->product_type) }}" style="display: none;">
...
<script>
    ...
    const selectedCategories = @json($product->categories->pluck('id'));
    let arrIds = [];
    ...
    // Automatically fetch subcategories for checked items
    arrIds.forEach(id => {
        const checkbox = document.getElementById(`checkbox-category-id-${id}`);
